...
Configure the connection string in appsettings.json under ConnectionStrings:DefaultConnection if you want to use a different SQL Server instance. By default the project uses a SQL Server connection named DefaultConnection.

From the project folder run:

dotnet build dotnet ef database update # applies migrations dotnet run

Open the site (usually at https://localhost:5001) and sign in.
...
Promoting a user to Admin If you need to promote an existing user to Admin, one option is to run SQL against the app database (use caution):

Find the user id and the role id (Admin) in the AspNetUsers / AspNetRoles tables.

Insert a record into AspNetUserRoles:
